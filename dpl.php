...
if ($section === "*") {
    fwrite(
        STDERR,
        "Error: [*] is a defaults section and cannot be deployed to." . PHP_EOL,
    );
    exit(1);
}
...
foreach (["host", "path"] as $required) {
    if (empty($sectionConfig[$required])) {
        fwrite(
            STDERR,
            "Error: \"$required\" is required in [$section] section of dpl.ini." .
                PHP_EOL,
        );
        exit(1);
    }
...
if (!empty($uploadFailed)) {
    fwrite(
        STDERR,
        PHP_EOL . "Failed to upload (" . count($uploadFailed) . "):" . PHP_EOL,
    );
...
if (!empty($deleteFailed)) {
    fwrite(
        STDERR,
        PHP_EOL . "Failed to delete (" . count($deleteFailed) . "):" . PHP_EOL,
    );
